...
4. Add these environment secrets exactly:
   ...
   - `APPLE_API_ISSUER_ID`;
   - `APPLE_TEAM_ID`;
   - `SPARKLE_PRIVATE_KEY` — the exported Ed25519 private key for the
     `com.pocarles.reserve` Sparkle account. Keep its Keychain original and an
     offline backup; never print or commit it.
...
- [ ] Bundle identity is `com.pocarles.reserve`, version is the intended stable
      X.Y.Z value, and the signed update feed targets this repository only.
...
Confirm `appcast.xml` names the exact release DMG and contains a
`sparkle:edSignature` generated by the protected workflow.
...
4. Download the public `Reserve.dmg`, `Reserve.dmg.sha256`, and `appcast.xml`;
   reproduce the checksum and fresh-install smoke test.
5. Install the previous public version, use **Check for Updates…**, and prove
   the signed in-app update replaces and relaunches it successfully.
6. Verify the landing page's latest-download URL.
